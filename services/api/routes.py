...
from core import pantry_service, recipe_service, cooking_service, nutrition_service, sample_data
...
async def pantry_service_scan_pantry_items(image: UploadFile = File(...), user_id: str = Form(...)) -> ScanPantryItemsOutputSchema:
    ...
    """
    # Download image from the client
    if image is not None:
        content_type = image.content_type or "application/octet-stream"
        contents = await image.read()
        file_size = len(contents)
        image = MediaFile(size=file_size, mime_type=content_type, bytes=contents)

    response = await run_sync_in_thread(pantry_service.scan_pantry_items, image=image, user_id=user_id)
    return response
...
async def pantry_service_add_pantry_item(body: BodyPantryServiceAddPantryItem = Body(...)) -> AddPantryItemOutputSchema:
    ...
    """
    response = await run_sync_in_thread(pantry_service.add_pantry_item, user_id=body.user_id, name=body.name, category=body.category, quantity=body.quantity, unit=body.unit, brand=body.brand, barcode=body.barcode, image_path=body.image_path, tags=body.tags)
    return response
...
async def pantry_service_get_user_pantry(body: BodyPantryServiceGetUserPantry = Body(...)) -> GetUserPantryOutputSchema:
    ...
    """
    response = await run_sync_in_thread(pantry_service.get_user_pantry, user_id=body.user_id)
    return response
...
async def pantry_service_update_pantry_item_quantity(body: BodyPantryServiceUpdatePantryItemQuantity = Body(...)) -> UpdatePantryItemQuantityOutputSchema:
    ...
    """
    response = await run_sync_in_thread(pantry_service.update_pantry_item_quantity, item_id=body.item_id, new_quantity=body.new_quantity)
    return response
...
async def pantry_service_remove_pantry_item(body: BodyPantryServiceRemovePantryItem = Body(...)) -> RemovePantryItemOutputSchema:
    ...
    """
    response = await run_sync_in_thread(pantry_service.remove_pantry_item, item_id=body.item_id)
    return response
...
async def pantry_service_search_pantry_items(body: BodyPantryServiceSearchPantryItems = Body(...)) -> SearchPantryItemsOutputSchema:
    ...
    """
    response = await run_sync_in_thread(pantry_service.search_pantry_items, user_id=body.user_id, query=body.query)
    return response
...
async def recipe_service_get_recipe_recommendations(body: BodyRecipeServiceGetRecipeRecommendations = Body(...)) -> GetRecipeRecommendationsOutputSchema:
    ...
    """
    response = await run_sync_in_thread(recipe_service.get_recipe_recommendations, user_id=body.user_id, preferences=body.preferences, dietary_restrictions=body.dietary_restrictions)
    return response
...
async def recipe_service_get_recipe_by_id(body: BodyRecipeServiceGetRecipeById = Body(...)) -> GetRecipeByIdOutputSchema:
    ...
    """
    response = await run_sync_in_thread(recipe_service.get_recipe_by_id, recipe_id=body.recipe_id)
    return response
...
async def recipe_service_create_base_recipe(body: BodyRecipeServiceCreateBaseRecipe = Body(...)) -> CreateBaseRecipeOutputSchema:
    ...
    """
    response = await run_sync_in_thread(recipe_service.create_base_recipe, name=body.name, description=body.description, cuisine_type=body.cuisine_type, difficulty_level=body.difficulty_level, prep_time_minutes=body.prep_time_minutes, cook_time_minutes=body.cook_time_minutes, servings=body.servings, base_ingredients=body.base_ingredients, base_steps=body.base_steps, branch_points=body.branch_points, dietary_tags=body.dietary_tags)
    return response
...
async def recipe_service_generate_recipe_from_ingredients(body: BodyRecipeServiceGenerateRecipeFromIngredients = Body(...)) -> GenerateRecipeFromIngredientsOutputSchema:
    ...
    """
    response = await run_sync_in_thread(recipe_service.generate_recipe_from_ingredients, ingredient_list=body.ingredient_list, cuisine_preference=body.cuisine_preference, dietary_restrictions=body.dietary_restrictions)
    return response
...
async def cooking_service_start_cooking_session(body: BodyCookingServiceStartCookingSession = Body(...)) -> StartCookingSessionOutputSchema:
    ...
    """
    response = await run_sync_in_thread(cooking_service.start_cooking_session, user_id=body.user_id, base_recipe_id=body.base_recipe_id)
    return response
...
async def cooking_service_make_cooking_decision(body: BodyCookingServiceMakeCookingDecision = Body(...)) -> MakeCookingDecisionOutputSchema:
    ...
    """
    response = await run_sync_in_thread(cooking_service.make_cooking_decision, session_id=body.session_id, branch_point=body.branch_point, choice=body.choice)
    return response
...
async def cooking_service_process_voice_question(body: BodyCookingServiceProcessVoiceQuestion = Body(...)) -> ProcessVoiceQuestionOutputSchema:
    ...
    """
    response = await run_sync_in_thread(cooking_service.process_voice_question, session_id=body.session_id, question=body.question)
    return response
...
async def cooking_service_add_cooking_photo(photo: UploadFile = File(...), session_id: UUID = Form(...), step_number: int = Form(...)) -> AddCookingPhotoOutputSchema:
    ...
    """
    # Download photo from the client
    if photo is not None:
        content_type = photo.content_type or "application/octet-stream"
        contents = await photo.read()
        file_size = len(contents)
        photo = MediaFile(size=file_size, mime_type=content_type, bytes=contents)

    response = await run_sync_in_thread(cooking_service.add_cooking_photo, session_id=session_id, photo=photo, step_number=step_number)
    return response
...
async def cooking_service_update_current_step(body: BodyCookingServiceUpdateCurrentStep = Body(...)) -> UpdateCurrentStepOutputSchema:
    ...
    """
    response = await run_sync_in_thread(cooking_service.update_current_step, session_id=body.session_id, step_number=body.step_number)
    return response
...
async def cooking_service_complete_cooking_session(body: BodyCookingServiceCompleteCookingSession = Body(...)) -> CompleteCookingSessionOutputSchema:
    ...
    """
    response = await run_sync_in_thread(cooking_service.complete_cooking_session, session_id=body.session_id, rating=body.rating, notes=body.notes, would_make_again=body.would_make_again)
    return response
...
async def cooking_service_get_cooking_session(body: BodyCookingServiceGetCookingSession = Body(...)) -> GetCookingSessionOutputSchema:
    ...
    """
    response = await run_sync_in_thread(cooking_service.get_cooking_session, session_id=body.session_id)
    return response
...
async def cooking_service_get_user_cooking_sessions(body: BodyCookingServiceGetUserCookingSessions = Body(...)) -> GetUserCookingSessionsOutputSchema:
    ...
    """
    response = await run_sync_in_thread(cooking_service.get_user_cooking_sessions, user_id=body.user_id)
    return response
...
async def nutrition_service_add_nutrition_data(body: BodyNutritionServiceAddNutritionData = Body(...)) -> AddNutritionDataOutputSchema:
    ...
    """
    response = await run_sync_in_thread(nutrition_service.add_nutrition_data, ingredient_name=body.ingredient_name, serving_size=body.serving_size, calories=body.calories, protein_g=body.protein_g, carbs_g=body.carbs_g, fat_g=body.fat_g, fiber_g=body.fiber_g, sugar_g=body.sugar_g, sodium_mg=body.sodium_mg, category=body.category, vitamin_c_mg=body.vitamin_c_mg, vitamin_a_iu=body.vitamin_a_iu, calcium_mg=body.calcium_mg, iron_mg=body.iron_mg, potassium_mg=body.potassium_mg, cost_per_serving=body.cost_per_serving, allergens=body.allergens, data_source=body.data_source)
    return response
...
async def nutrition_service_get_nutrition_data(body: BodyNutritionServiceGetNutritionData = Body(...)) -> GetNutritionDataOutputSchema:
    ...
    """
    response = await run_sync_in_thread(nutrition_service.get_nutrition_data, ingredient_name=body.ingredient_name)
    return response
...
async def nutrition_service_search_nutrition_data(body: BodyNutritionServiceSearchNutritionData = Body(...)) -> SearchNutritionDataOutputSchema:
    ...
    """
    response = await run_sync_in_thread(nutrition_service.search_nutrition_data, query=body.query)
    return response
...
async def nutrition_service_calculate_recipe_nutrition(body: BodyNutritionServiceCalculateRecipeNutrition = Body(...)) -> CalculateRecipeNutritionOutputSchema:
    ...
    """
    response = await run_sync_in_thread(nutrition_service.calculate_recipe_nutrition, ingredients=body.ingredients)
    return response
...
async def nutrition_service_estimate_nutrition_from_llm(body: BodyNutritionServiceEstimateNutritionFromLlm = Body(...)) -> EstimateNutritionFromLlmOutputSchema:
    ...
    """
    response = await run_sync_in_thread(nutrition_service.estimate_nutrition_from_llm, ingredient_name=body.ingredient_name, serving_size=body.serving_size)
    return response
...
async def nutrition_service_bulk_import_nutrition_data(body: BodyNutritionServiceBulkImportNutritionData = Body(...)) -> BulkImportNutritionDataOutputSchema:
    ...
    """
    response = await run_sync_in_thread(nutrition_service.bulk_import_nutrition_data, nutrition_list=body.nutrition_list)
    return response
...
async def nutrition_service_get_nutrition_by_category(body: BodyNutritionServiceGetNutritionByCategory = Body(...)) -> GetNutritionByCategoryOutputSchema:
    ...
    """
    response = await run_sync_in_thread(nutrition_service.get_nutrition_by_category, category=body.category)
    return response
...
async def nutrition_service_update_nutrition_cost(body: BodyNutritionServiceUpdateNutritionCost = Body(...)) -> UpdateNutritionCostOutputSchema:
    ...
    """
    response = await run_sync_in_thread(nutrition_service.update_nutrition_cost, ingredient_name=body.ingredient_name, cost_per_serving=body.cost_per_serving)
    return response
...
async def nutrition_service_analyze_dietary_compatibility(body: BodyNutritionServiceAnalyzeDietaryCompatibility = Body(...)) -> AnalyzeDietaryCompatibilityOutputSchema:
    ...
    """
    response = await run_sync_in_thread(nutrition_service.analyze_dietary_compatibility, ingredients=body.ingredients, dietary_restrictions=body.dietary_restrictions)
    return response
...
async def sample_data_create_sample_recipes() -> CreateSampleRecipesOutputSchema:
    ...
    """
    response = await run_sync_in_thread(sample_data.create_sample_recipes)
    return response
...
async def sample_data_create_sample_nutrition_data() -> CreateSampleNutritionDataOutputSchema:
    ...
    """
    response = await run_sync_in_thread(sample_data.create_sample_nutrition_data)
    return response
